    ...
    <div class='relative h-screen bg-white dark:bg-zinc-800 text-center xl:bg-zinc-200'>
        ...
        <div
            ...
            <vee-form @submit="onRegisterHandler">
                <!-- 用户名 -->
                <vee-field v-model="registerForm.username"
                    ...
                    name="username" placeholder="用户名" autocomplete="on" type="text" :rules="validateUsername" />
                <vee-error-message class="text-sm text-red-600 block mt-0.5 text-left" name="username" />
                <!-- 密码 -->
                <vee-field v-model="registerForm.password"
                    class="dark:bg-zinc-800 dark:text-zinc-400 border-b-zinc-400 border-b w-full outline-0 pb-1 px-1 text-base focus:border-b-main dark:focus:border-b-zinc-200 xl:default:bg-zinc-900"
                    name="password" placeholder="密码" autocomplete="on" type="password" :rules="validatePassword" />
                <vee-error-message class="text-sm text-red-600 block mt-0.5 text-left" name="password" />
                <!-- 确认密码 -->
                <vee-field v-model="registerForm.confirmPassword"
                    class="dark:bg-zinc-800 dark:text-zinc-400 border-b-zinc-400 border-b w-full outline-0 pb-1 px-1 text-base focus:border-b-main dark:focus:border-b-zinc-200 xl:default:bg-zinc-900"
                    name="confirmPassword" placeholder="确认密码" autocomplete="on" type="password"
                ...
                <div class="pt-1 pb-3 leading-[0px] text-right">
                    <a @click="onToLogin"
                        class=" inline-block p-1 text-zinc-400 text-right dark:text-zinc-600 dark:hover:text-zinc-400 text-sm duration-300 cursor-pointer">去登录</a>
                </div>
                <!-- 注册协议 -->
                <div class="text-center">
                    <a class="text-zinc-400 dark:text-zinc-600 hover:text-zinc-600 dark:hover:text-zinc-400 text-sm duration-300"
                        href="https://gaojianghua.cn/agreement" target="_blank">
                        注册即同意 《wolffy协议》
                    </a>
                </div>
                <!-- 注册按钮 -->
                <g-button :loading="loading" :isActiveAnim="false" class="w-full dark:bg-zinc-900 xl:dark:bg-zinc-800">
                    立即注册</g-button>
            </vee-form>
        </div>
    </div>
