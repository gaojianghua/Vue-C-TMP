    ...
    <g-popover class="flex items-center" placement="bottom-left">
        ...
        <template #reference>
            <div v-if="useStore().user.token"
                ...
                <!-- 用户头像 -->
                <img v-lazy class="w-3 h-3 rounded-sm" :src="useStore().user.userInfo.avatar" alt="" />
                <!-- 下箭头 -->
                <g-svg-icon class="h-1.5 w-1.5 ml-0.5" name="down-arrow" fillClass="fill-zinc-900 dark:fill-zinc-200">
                </g-svg-icon>
                <!-- vip图标 -->
                <g-svg-icon v-if="useStore().user.userInfo.vipLevel" class="h-1.5 w-1.5 absolute right-[16px] bottom-0"
                    name="vip" fillClass="fill-zinc-900 dark:fill-zinc-300"></g-svg-icon>
            </div>
            <div v-else>
                <g-button class="guide-my" icon="profile" iconColor="#fff" @click="onToLogin"></g-button>
            </div>
        </template>
        <!-- 匿名插槽 -->
        <div v-if="useStore().user.token" class="w-[140px] p-1 overflow-hidden">
            <div v-for="item in userArr" :key="item.id" @click="onItemClick(item)"
