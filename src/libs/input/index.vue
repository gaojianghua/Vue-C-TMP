<template>
    <div class=' relative leading-[0px]'>
        ...
        <input
            ...
            v-if="type === TYPE_TEXT" type="text" v-model="text" :maxlength="max" />
        <!-- 多行 -->
        <textarea
            ...
            v-if="type === TYPE_TEXTAREA" v-model="text" rows="5" :maxlength="max"></textarea>
        <!-- 最大长度 -->
        <span v-if="max" class=" absolute right-1 bottom-0.5 text-zinc-400 text-xs"
            :class="{ 'text-red-700': currentNumber === parseInt(String(max)) }">{{ currentNumber }} / {{ max }}</span>
    </div>
</template>
