<template>
    <div class='h-full bg-zinc-200 dark:bg-zinc-800 duration-500 overflow-auto xl:pt-1 scrollbar-none'>
        <div
            ...
            <!-- 移动端的 navbar -->
            <g-navbar sticky v-if="isMobileTerminal" :clickLeft="onNavbarLeftClick">个人资料</g-navbar>
            <!-- PC 端标题 -->
            ...
            <div class="h-full w-full px-1 pb-4 text-sm mt-2 xl:w-2/3 xl:text-center">
                ...
                <div class="py-1 xl:absolute xl:right-[16%] xl:text-center">
                    <span
                        class="w-8 inline-block mb-2 font-black text-sm dark:text-zinc-300 xl:block xl:mx-auto">我的头像</span>
                    <div @click="onAvatarClick"
                        class=" relative w-[80px] h-[80px] group xl:cursor-pointer xl:left-[50%] xl:translate-x-[-50%]">
                        <!-- 头像 -->
                        <img class=" rounded-full w-full h-full xl:inline-block" :src="useStore().user.userInfo.avatar"
                            alt="">
                        <!-- 鼠标移入 -->
                        <div class=" absolute top-0 rounded-full w-full h-full bg-black/40 hidden xl:group-hover:block">
                            <g-svg-icon name="chang-header-image" class="w-2 h-2 m-auto mt-2" color="#fff"></g-svg-icon>
                ...
                <div class=" py-1 xl:flex xl:items-center xl:my-1">
                    ...
                    <g-input class="w-full" type="textarea" max="50" v-model="userInfo.introduction">
                    </g-input>
                </div>
                <!-- 保存修改 -->
                <g-button :loading="loading" @click="onChangeProfile"
                    class="w-full mt-2 mb-4 dark:text-zinc-300 dark:bg-zinc-800 xl:w-[160px] xl:ml-[50%] xl:translate-x-[-50%]">
                    保存修改</g-button>
                <!-- 移动端: 退出登录 -->
                <g-button v-if="isMobileTerminal" @click="onLogoutClick"
